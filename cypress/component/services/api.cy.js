...
describe(SimpleWeatherAPI.name, function () {
    ...
    context("hourly weather data", function () {
        beforeEach(function () {
            ...
            cy.get(`@inputData`).then((inputData) => {
                ...
                const indexOfCurrentHour = inputData.hourly.time.findIndex((t) =>
                    gte(
                        DateTime.fromISO(t).toMillis(),
                        DateTime.fromISO(currentTime).startOf("hour").toMillis()
                    )
                );
            ...
            cy.get(`@currentTime`).then((currentTime) => {
                cy.get(`@inputData`).then(({ hourly }) => {
                    cy.get(`@indexOfCurrentHour`).then((indexOfCurrentHour) => {
                        ...
                        const filteredHourlyTimes = hourly.time.slice(
                            indexOfCurrentHour,
                            indexOfCurrentHour + 25
                        );
                    ...
                    cy.get(`@indexOfCurrentHour`).then((indexOfCurrentHour) => {
                        hourlyWeather.forEach((actualHour, i) => {
                            ...
                            Object.keys(hourly).forEach((k) => {
                                expect(actualHour.values[k]).to.eq(
                                    hourly[k][inputDataHourlyMappedIndex]
                                );
                            });
                        });
                    });
    ...
    context("daily weather data", function () {
        ...
        it("contains only the first 7 entries which begin from the current day", function () {
            ...
            cy.get(`@inputData`).then((inputData) => {
                const currentDay = DateTime.fromISO(inputData.current.time)
                    .startOf("day")
                    .toISODate();
        ...
        context("mappings", function () {
            ...
            specify("sunrise", () => {
                cy.get(`@dailyWeather`).then((dailyWeather) => {
                    expect(dailyWeather[0].mapped.sunrise.toISO({ includeOffset: false })).to.eq(
                        `2023-11-14T07:26:00.000`
                    );
                });
            });
            ...
            specify("sunset", () => {
                cy.get(`@dailyWeather`).then((dailyWeather) => {
                    expect(dailyWeather[0].mapped.sunset.toISO({ includeOffset: false })).to.eq(
                        `2023-11-14T16:15:00.000`
                    );
                });
            });
        });
